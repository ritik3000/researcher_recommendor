...
# In[83]:
alpha=[0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.9,1]
...
percentage_collab_score=pickle.load(open("pickles/percentage_collab_score.pkl","rb"))
for alp in alpha:
    for bet in beta:
        print("alpha {:f} beta {:f} ".format(alp,bet))
        ...
        for i in range(len(loa)):
            for j in range(len(loa)):
                if((loa[i],loa[j]) in percentage_collab_score):
                    cos[i][j]=bet*cos[i][j]+(1-bet)*percentage_collab_score[(loa[i],loa[j])]
        ...
        def graphrec(rwralpha,name):
            ...
            q = s
            for i in range(50):
                s = numpy.dot(rwralpha,(numpy.dot(s,cos)))+(1-rwralpha)*numpy.array(q)
                #print s[:10]
            rec = []
            for i in range(len(loa)):
        ...
        if (diff/120)>=max_diff:
            print("Changed:",diff/120-max_diff)
            max_diff=diff/120
            max_alpha=alp
            ma_f1=f1
        f1_scores.append([alp,bet,diff/120])


print(max_alpha)
print(max_diff)
print(ma_f1)
pickle.dump(ma_f1,open("max_f1_rwr_alpha_beta_percentage_collab.pkl","wb"))
...
out = open('percentage_collab_score_rwralpha_beta.csv', 'w')
